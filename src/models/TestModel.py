...
class TestModel(nn.Module):
    ...
    def __init__(self, num_classes=5, in_channels=6, dropout=0.15):
        super().__init__()

        # ── Bloc CNN — extraction de features spatiales ───────────
        self.cnn = nn.Sequential(
            # Bloc 1
            nn.Conv2d(in_channels, 32, kernel_size=(3,128), padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),        # (6,128,259) → (32,64,67)
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),        # (32,64,67) → (64,32,33)
            nn.Dropout2d(dropout),
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d((2, 1)),      # (64,32,33) → (128,16,33)
            nn.Dropout2d(dropout)
        )

        # ── BiLSTM — capture des patterns temporels ───────────────
        # Entrée : (batch, time=33, features=128*16=2048)
        ...
        self.bilstm = nn.LSTM(
            ...
            num_layers=2,
            batch_first=True,
            bidirectional=True,       # -> sortie : (batch, time=33, 256)
            dropout=dropout
        )
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)            # (batch, 33, 128, 16)
        x = x.reshape(batch, W, C * H)       # (batch, 33, 2048)
        ...
        # ── Attention ─────────────────────────────────────────────
        scores  = self.attention(x)            # (batch, 33, 1)
...
if __name__ == "__main__":
    model = TestModel(num_classes=5, in_channels=6)
